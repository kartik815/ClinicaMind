...
// Simple PNG generator in pure Node without native dependencies
function createSimplePNG(width, height, title, subtitle) {
  // SVG markup for crisp high resolution EMR preview
  const svg = `
...
const screenshots = [
  { name: 'Dashboard.svg', pngName: 'Dashboard.png', title: 'Clinical Operations Dashboard', subtitle: "Today's Appointments, Waiting Patients, Emergency Alerts & AI Feed" },
  { name: 'Patients.svg', pngName: 'Patients.png', title: 'Patients Directory & Digital Folders', subtitle: 'Permanent Medical Profiles, Risk Stratification & File Uploads' },
  { name: 'PatientProfile.svg', pngName: 'PatientProfile.png', title: 'Patient Digital Folder Overview', subtitle: 'Demographics, Lifestyle, Allergies, Medications, Imaging & Lab Reports' },
  { name: 'VisitHistory.svg', pngName: 'VisitHistory.png', title: 'Immutable Visit History Records', subtitle: 'Chronological Consultation Records, Diagnosis & Prescriptions' },
  { name: 'Consultation.svg', pngName: 'Consultation.png', title: "Today's Consultation Queue", subtitle: 'Triage Queue: Waiting, Active, Completed & Emergency Patients' },
  { name: 'AIWorkspace.svg', pngName: 'AIWorkspace.png', title: 'AI Clinical Workspace (React Flow)', subtitle: 'Multi-Agent Collaborative Visual Intelligence Canvas' },
  { name: 'Reports.svg', pngName: 'Reports.png', title: 'Clinical Reports & PDF Generator', subtitle: 'Auto-Generated Physician Summaries, Prescriptions & Discharge Care Plans' }
];

screenshots.forEach(s => {
  const svgContent = createSimplePNG(1280, 720, s.title, s.subtitle);
  fs.writeFileSync(path.join(targetDir, s.name), svgContent);
  // Also save SVG as PNG placeholder for viewer compatibility
  fs.writeFileSync(path.join(targetDir, s.pngName), svgContent);
});

console.log('Successfully generated all 7 progress screenshots in /progress_screenshots/!');
